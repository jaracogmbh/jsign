...
## Was hat sich geändert im Open-Source-Code
Es wurden folgende Änderungen am Originalcode von ``jsign`` vorgenommen:
### Änderungen
- Änderung der Provider Registrierung in der ``SigningServiceJcaProvider`` Klasse.
  - damit ein neuer Provider registriert werden kann, wurde die Klasse so geändert, dass dynamisch neue Provider registriert werden können.
- Einführung eines neuen ``KeyStoreType``: ``CUSTOMPROVIDER``.
  ...
  - dieser implementiert die für den ``KeyStoreType`` benötigten Methoden ``validate`` und ``getProvider``.
- Verifizierung der signierten Datei im ``AuthenticodeSigner`` wurde abgeschaltet.
  - da die Signatur über eine externe API erstellt wird, ist es einerseits nicht notwendig die Signatur zu verifizieren und andererseits hat nur die API die benötigten Informationen um diesen zu verifizieren welche nicht für ``jsign`` zur Verfügung stehen.
...
- Test Klassen.
  - es wurden Test Klassen erstellt, um die Funktionalität sicherzustellen.
...
2. Über den ``storetype`` Parameter, weiß der ``KeyStoreType``, welchen Provider er verwenden soll. In diesem Fall ist es der ``CUSTOMPROVIDER``.
   ...
   - dann wird der benötigte Provider mit den richtigen ``SigningService`` initiiert.
3. im ``CustomProviderSigningService`` wird die Datei über die API signiert.
   - dazu wird zu erst das benötigte Zertifikat über die API ermittelt.
     ...
     - die API liefert dann ein Zertifikat zurück.
...
1. Über den ``storepass`` Parameter
   - dort kann das Passwort als letzter Parameter direkt mit übergeben werden
...
- ``non decorate signature``: Non Decorate Signature
  - hier werden nur folgende Werte als Eingabe akzeptiert:
...
```
Außerdem ist zu beachten, dass leere Werte (``||``) nicht erlaubt sind. Hier wird dann ein Fehler geworfen und das Programm wird beendet. Auch müssen alle Werte gesetzt werden.
...
### Endpunkt
Sowohl das Zertifikat als auch die Signatur werden über eine API ermittelt bzw. erstellt. Der Endpunkt wird über den Parameter ``keysotre`` übergeben. Der Endpunkt ist dabei nur der Hostname und der Port (oder die Domain). Der Pfad der Endpunkte für die Zertifikate und Signaturen sind im Programm festgelegt. 
...
```
### Ausführung mit ``jarsigner`` über die Kommandozeile
...
- zu beachten:
  ...
  - der Endpunkt wird über den ``providerArg`` Parameter übergeben
    ...
    - außerdem ist dieser Parameter nicht nativ in ``jsign`` vorhanden, sondern nur in Kombination mit ``jarsigner``.
- Zusätzliche Flags:
    - ``sigalg``: Signatur Algorithmus
    - ``digestalg``: Digest Algorithmus
    - diese beiden Flags müssen gesetzt werden, da ``jarsigner`` sonst nicht den richtigen Algorithmus für die Signatur benutzt.
